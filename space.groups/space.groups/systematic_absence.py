...
class SystematicAbsence(object):
    ...
    def __init__(self, pattern, condition, divisor):
        ...
        if (self.pattern.size == 9):
            if (self.pattern.shape != (3,3)):
                self.pattern = self.pattern.reshape((3,3))
            return
        elif (self.pattern.size == 16):
            if (self.pattern.shape != (4,4)):
                self.pattern = self.pattern.reshape((4,4))
            return
        else:
            raise MatrixException("Expecting 9 or 16 elements to make square matrix pattern. Got "+str(self.pattern.size)+" elements.")
