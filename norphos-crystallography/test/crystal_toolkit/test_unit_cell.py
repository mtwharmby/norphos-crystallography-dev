...
class TestUnitCell(unittest.TestCase):
    ...
    def test_lattice_system(self):
        #Cubic a = 3
        self.uc = UnitCell(Lattice(3))
        assert_equal(CrystalSystem.CUBIC, self.uc.lattice.lattice_system)
        assert_equal(CrystalSystem.CUBIC, self.uc.crystal_family)

        #Rhombohedral a = 3; al = 60
        self.uc.update_cell(Lattice(3, al=60))
        assert_equal(CrystalSystem.RHOMBOHEDRAL, self.uc.lattice.lattice_system)
        assert_equal(CrystalSystem.HEXAGONAL, self.uc.crystal_family)

        #Hexagonal a = 5; c = 2; ga = 120
        self.uc.update_cell(Lattice(5,c=2,ga=120))
        assert_equal(CrystalSystem.HEXAGONAL, self.uc.lattice.lattice_system)
        assert_equal(CrystalSystem.HEXAGONAL, self.uc.crystal_family)

        #Tetragonal a = b = 2; c = 5
        self.uc.update_cell(Lattice(2, 5))
        assert_equal(CrystalSystem.TETRAGONAL, self.uc.lattice.lattice_system)
        assert_equal(CrystalSystem.TETRAGONAL, self.uc.crystal_family)

        #a = 5; b = 3; c = 2; angles 90
        self.uc.update_cell(Lattice(5, 3, 2))
        assert_equal(CrystalSystem.ORTHORHOMBIC, self.uc.lattice.lattice_system)
        assert_equal(CrystalSystem.ORTHORHOMBIC, self.uc.crystal_family)

        #a = 2; b = 3; c = 5; be = 30 al = ga = 90
        self.uc.update_cell(Lattice(2, 3, 5, be=30))
        assert_equal(CrystalSystem.MONOCLINIC, self.uc.lattice.lattice_system)
        assert_equal(CrystalSystem.MONOCLINIC, self.uc.crystal_family)

        #a = 3; b = 5; c = 2; al = 30deg; be = 45 deg; ga = 60 deg
        self.uc.update_cell(Lattice(3, 5 ,2, 30, 45, 60))
        assert_equal(CrystalSystem.TRICLINIC, self.uc.lattice.lattice_system)
        assert_equal(CrystalSystem.TRICLINIC, self.uc.crystal_family)
